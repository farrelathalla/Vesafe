...
export MEDSENT_SERVER_NAME="${MEDSENT_SERVER_NAME:-$(json_default serverName VeSafeFHIR)}"
...
export MEDSENT_WALLET_COLLECTION="${MEDSENT_WALLET_COLLECTION:-$(json_default walletCollection VeSafe)}"
export MEDSENT_SERVICE_ROLE="${MEDSENT_SERVICE_ROLE:-VeSafe_Service}"

echo "Initializing InterSystems IRIS for VeSafe"
...
if classPath="" write !,"[VeSafe] BOOTSTRAP_CLASS_PATH is not set",! halt
set sc=$SYSTEM.OBJ.Load(classPath,"ck")
if $SYSTEM.Status.IsError(sc) write !,"[VeSafe] Unable to load bootstrap class from ",classPath,!
if $SYSTEM.Status.IsError(sc) do $SYSTEM.Status.DisplayError(sc)
if $SYSTEM.Status.IsError(sc) halt
do ##class(VeSafe.Installer).Main()
halt
EOF
)"
...
if [[ "${session_output}" != *"[VeSafe] Bootstrap finished successfully"* ]]; then
  echo "VeSafe IRIS bootstrap failed" >&2
  exit 1
fi

echo "VeSafe IRIS bootstrap complete"
