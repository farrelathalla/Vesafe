...
def test_create_facility(client, monkeypatch):
    async def fake_geocode(address, api_key):
        return {
            "address": "Jl. Pajajaran No.29, Bandung, Jawa Barat 40171, Indonesia",
            "lat": -6.9175,
            "lng": 107.6010,
            "google_place_id": "place-kimiafarma",
        }
    ...
    r = client.post("/api/facilities", json={
        ...
        "unit_name": "Lantai Produksi Utama",
    })
    assert r.status_code == 200
    payload = r.json()
    assert payload["google_place_id"] == "place-kimiafarma"
    assert payload["address"] == "Jl. Pajajaran No.29, Bandung, Jawa Barat 40171, Indonesia"
...
def test_video_extract_endpoint_processes_without_invalid_status(client, fresh_iris, monkeypatch):
    ...
    assert model.caption == "PT. Kimia Farma — Pabrik Bandung walkthrough model"
